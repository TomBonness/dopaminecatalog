...
export default function TrackingPage() {
  ...
  const deliveryComplete = activeOrder?.status === "completed" || (activeOrder?.deliveryProgress ?? 0) >= 100;

  useEffect(() => {
    if (!activeOrder || !deliveryComplete) return;
    router.replace("/mukbang");
  }, [activeOrder, deliveryComplete, router]);
  ...
  const progress = activeOrder.deliveryProgress;
  const isCompleted = deliveryComplete;
      ...
      <div className="flex flex-col items-center gap-6 w-full max-w-sm mx-auto">
        ...
        {!isCompleted ? (
          ...
          <motion.div
            ...
            className="w-full p-5 rounded-2xl bg-zinc-900 border border-zinc-800 space-y-4 text-center"
          >
            <div className="p-3.5 rounded-xl bg-neon-green/10 border border-neon-green/30 text-neon-green text-xs font-bold animate-pulse">
              🎉 Delivery capsule secured!
            </div>
            <div className="space-y-1">
              <h3 className="font-black text-sm uppercase tracking-wider text-white">Delivery capsule secured!</h3>
              <p className="text-[11px] text-zinc-400 leading-relaxed">
                Opening MukBang Studio...
              </p>
            </div>
            <Link href="/mukbang" className="block w-full">
              <button
